...
2. Go to your repository at [collective/training](https://github.com/collective/training).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `training.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | training.example.com | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/collective/training/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/collective/training/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/collective/training-backend`) and Frontend (`ghcr.io/collective/training-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of training.example.com](https://github.com/collective/training/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/training.example.com.yml` stack.
